...
public class DefaultSessionUtilTest {
  ...
  public void testGenerateRequestToken() {
    ...
    long epoch = sessionUtil.getThirtySecondsStepsFromEpoch();
    byte[] sessionToken = sessionUtil.generateSessionToken(key, ByteBuffer.wrap(destination.getBytes()), epoch);
    
    int r1 =
        sessionUtil.generateRequestToken(sessionToken, ByteBuffer.wrap("a new request".getBytes()), epoch);
    ...
    clock.setTime(60000);
    int r4 =
        sessionUtil.generateRequestToken(sessionToken, ByteBuffer.wrap("a new request".getBytes()), epoch + 2);
  ...
  public void testValidateMessage() {
    ...
    int requestToken = sessionUtil.generateRequestToken(sessionToken, ByteBuffer.wrap(message), epoch + 1);

    boolean valid =
        sessionUtil.validateMessage(sessionToken, ByteBuffer.wrap(message), requestToken, epoch + 1);
    Assert.assertTrue(valid);

    clock.setTime(40000);
    valid = sessionUtil.validateMessage(sessionToken, ByteBuffer.wrap(message), requestToken, epoch + 2);
