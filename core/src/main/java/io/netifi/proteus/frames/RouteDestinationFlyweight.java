...
public class RouteDestinationFlyweight {
  private static final int LAST_MASK = 0x80; //0b1000_0000;
  private static final int REMOVE_FLAG = 0x7F; //0b0111_1111;
  ...
  public static int computeLength(RouteType routeType, CharSequence group) {
  ...
  public static int computeLength(RouteType routeType, CharSequence destination, CharSequence group) {
  ...
  public static int encodeRouteByDestination(
      ByteBuf byteBuf, RouteType routeType, long accountId, CharSequence destination, CharSequence group) {
    return encode(byteBuf, routeType, accountId, destination, group);
  }

  public static int encodeRouteByGroup(
      ByteBuf byteBuf, RouteType routeType, long accountId, CharSequence group) {
  ...
  private static int encode(
    ...
    if (hasDestination) {
      ...
      byteBuf.setCharSequence(offset, destination, CharsetUtil.US_ASCII);
    ...
    byteBuf.setCharSequence(offset, group, CharsetUtil.US_ASCII);
    offset += groupLength;
