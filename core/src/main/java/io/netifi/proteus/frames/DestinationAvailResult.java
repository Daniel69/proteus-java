...
import io.netifi.proteus.util.StringUtil;
...
public class DestinationAvailResult {
  ...
  public static int computeLength(CharSequence destination) {
    return FrameHeaderFlyweight.computeFrameHeaderLength() + DESTINATION_LENGTH_SIZE + FOUND_SIZE + destination.length();
  }

  public static int encode(ByteBuf byteBuf, CharSequence destination, boolean found, long seqId) {
